...
for amp in stim_amps:
    ...
    v_vec = h.Vector().record(pfc_cell.soma(0.5)._ref_v)
...
for i, amp in enumerate(stim_amps):
    axs[i].plot(all_t[i], all_v[i], color='darkorange') #, label=f'amp={amp} nA')
...
# fig.suptitle(f'PFC pyramidal neuron response, psychedelic effect scaling factor = {ps_factor}')
